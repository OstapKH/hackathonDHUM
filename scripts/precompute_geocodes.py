...
OUTPUT_FILE = '../data/city_coordinates.csv'
...
def precompute_city_coordinates():
    ...
    # Load metadata
    try:
        df = pd.read_csv('../data/PluG2_metadata.psv', sep='|', low_memory=False)
        logger.info(f"Loaded {len(df)} records from metadata file.")
    except Exception as e:
        logger.error(f"Could not load metadata file '../data/PluG2_metadata.psv': {e}")
        return
